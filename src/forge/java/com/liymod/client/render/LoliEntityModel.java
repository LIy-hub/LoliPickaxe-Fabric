...
/** Forge 1.20.1 renderer model preserving the upstream Loli geometry. */
public final class LoliEntityModel extends EntityModel<LoliEntity> {
    public static final ModelLayerLocation LAYER = new ModelLayerLocation(
            new ResourceLocation(LiyMod.MOD_ID, "loli"), "main");
    ...
    @Override
    public void setupAnim(LoliEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks,
                          float netHeadYaw, float headPitch) {
        float amount = limbSwingAmount * 1.5F;
        leftArm.xRot = Mth.cos(limbSwing + Mth.PI) * amount;
        rightArm.xRot = Mth.cos(limbSwing) * amount;
        leftLeg.xRot = Mth.cos(limbSwing) * amount;
        rightLeg.xRot = Mth.cos(limbSwing + Mth.PI) * amount;
