...
public final class BugEntityClearItem extends Item {
    public BugEntityClearItem(Properties properties) { super(properties); }
    @Override public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        if (player.level().isClientSide) target.remove(Entity.RemovalReason.DISCARDED);
    ...
    @Override public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.translatable("item.liymod.bug_entity_clear.warning"));
