...
- `force_remove` 是默认关闭的旧版强制移除兼容开关。冻结的主动绝对处决本就会
  ...
  `discard` 或 `remove`。
- `clear_inventory` 与 `drop_equipment` 在绝对处决票据建立后、原版死亡掉落前
  事务式移出精确槽位与鼠标携带栈；任一免疫、撤销或异常分支都会原槽回滚并同步
  菜单，只有最终 `DEAD_LOCK` 成功才提交为目标归属、无敌、无限寿命的可回收
  掉落。`kick_player`、名单写入也只在同一提交点发生；踢出消息限 160 字符并
  移除换行和格式控制符。
